...
export const createPlanSuiDeFiAgentStrategyTool = (userId: string) =>
  tool({
    description:
      "Plan an autonomous Sui DeFi/Agentic Web strategy for Barzakh's embedded Sui wallet. Covers LP/yield scouting, and Walrus memory. This is a planning/risk tool, not an execution tool.",
    parameters: z.object({
      strategy: z.enum(["lp", "walrus"]),
      ...
      objective: z.string().optional().describe("User objective, e.g. leverage LP yield then upload memory to Walrus."),
