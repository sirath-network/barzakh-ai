...
fn orbGlassLiquidAnim(uv01: vec2<f32>) -> vec4<f32> {
  ...
  let ballA = 1.0 - smoothstep(0.98, 1.0, pd);
  col = clamp(col * max(u.exposure, 0.0), vec3<f32>(0.0), vec3<f32>(1.0));
  return vec4<f32>(col, ballA);
}
...
fn fs_main(in: VOut) -> @location(0) vec4<f32> {
  ...
  // Exact analytic circular boundary with clean antialiasing
  let pixelFeather = 3.0 / max(min(u.size.x, u.size.y), 1.0);
  let feather = max(pixelFeather, 0.015);
  let alpha = 1.0 - smoothstep(1.0 - feather, 1.0, pd);

  // Strictly discard any pixel outside or on the zero threshold to eliminate dots completely
  if (alpha <= 0.005) {
  ...
  // Pure premultiplied alpha: RGB is multiplied by alpha
  return vec4<f32>(rgb * alpha, alpha);
}
`;

const uniformSeed = [
  1, 1, 0, 2, 0.7200000286102295, 0.36000001430511475, 3.799999952316284, 0.4399999976158142,
  5.199999809265137, 0.5799999833106995, 0.36000001430511475, 0.2800000011920929, 0.20000000298023224, 0.2199999988079071, 1.0800000429153442, 12,
  0.004999999888241291, 0, 0, 1, 0.41999998688697815, 0, 2, 0.41999998688697815,
  ...
  0.20392157137393951, 0.22745098173618317, 0.26274511218070984, 1, 0.0117647061124444, 0.01568627543747425, 0.019607843831181526, 1,
  1, 1, 1, 1, 1, 1, 1, 1,
  0.7254902124404907, 0.7529411911964417, 0.7921568751335144, 1, 1, 1, 1, 1,
  0.9176470637321472, 0.95686274766922, 1, 1, 0.8627451062202454, 0.9176470637321472, 1, 1,
  0.019607843831181526, 0.0235294122248888, 0.0313725508749485, 1, 1, 1, 1, 1,
...
// Module-level singleton device & pipeline caching for 0ms instant start
let globalGpuPromise: Promise<{ device: any; format: any; pipeline: any } | null> | null = null;

function getGlobalGpu() {
  if (typeof window === "undefined" || typeof navigator === "undefined" || !("gpu" in navigator) || !(navigator as any).gpu) {
    return null;
  }
  if (!globalGpuPromise) {
    globalGpuPromise = (async () => {
      try {
  ...
  return globalGpuPromise;
}

export const LiquidOrb: React.FC<LiquidOrbProps> = ({ className, size = 24 }) => {
  ...
  useEffect(() => {
    ...
    async function initWebGPU() {
      const gpuDataPromise = getGlobalGpu();
      if (!gpuDataPromise) {
        setHasWebGPU(false);
        return;
      }

      const gpuData = await gpuDataPromise;
      if (!gpuData || stopped || !canvas) {
        if (!gpuData) setHasWebGPU(false);
        return;
      }

      const { device, format, pipeline } = gpuData;
      ...
      try {
        ...
        const bufferUsage = gpuUsage
          ? (gpuUsage.UNIFORM | gpuUsage.COPY_DST)
          : 0x0040 | 0x0008;
        ...
        function frame(now: number) {
          ...
          try {
            ...
            const renderSize = Math.max(160, Math.floor(size * dpr));
        ...
        console.warn("WebGPU initialization failed, falling back to CSS orb:", err);
        setHasWebGPU(false);
      }
    }
  ...
  return (
    <div
      className={cn(
        "relative rounded-full overflow-hidden flex-shrink-0 select-none bg-transparent",
        className
      )}
    ...
    >
      ...
      <canvas
        ref={canvasRef}
        className={cn(
          "w-full h-full block rounded-full bg-transparent transition-opacity duration-300",
          hasWebGPU === true ? "opacity-100" : "opacity-0"
        )}
        style={{ width: size, height: size }}
        aria-label="Liquid Orb"
      />

      {/* Modern, sleek Fallback CSS Animated Orb when WebGPU is loading or unavailable */}
      {hasWebGPU !== true && (
        <div className="absolute inset-0 rounded-full flex items-center justify-center pointer-events-none bg-black/40 border border-white/10">
          {/* Spinning gradient border illusion */}
          <div 
            className="w-full h-full rounded-full animate-spin border-2 border-transparent border-t-blue-500 border-r-purple-500"
            style={{
          ...
          {/* Pulsing center glow */}
          <div 
            className="absolute rounded-full animate-pulse bg-gradient-to-tr from-blue-400 to-purple-500"
            style={{
              width: "60%",
              height: "60%",
              opacity: 0.8,
              boxShadow: "0 0 10px rgba(120,100,255,0.6)",
