...
interface NadFunSearchResult {
    status: string;
    count?: number;
    total_available?: number;
    results?: NadFunTokenResult[];
    message?: string;
    query?: string;
}
...
const NadFunTokenSearch: React.FC<{ result: NadFunSearchResult }> = ({ result }) => {
    ...
    if (!result || result.status === "error") {
        return (
            <div className="text-muted-foreground p-4 bg-muted/50 rounded-lg border border-border/20">
                {result?.message || "Failed to search nad.fun tokens."}
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-2xl mb-6">
            ...
            <div className="flex items-center justify-between mb-3 px-1">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                        nad.fun {result.query ? `• ${result.query}` : "Tokens"}
                    </span>
                </div>
            ...
            <div className="flex flex-col gap-2">
                {visibleTokens.map((token, index) => (
                    <a
                        ...
                        href={token.url || `https://nad.fun/tokens/${token.address}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group flex items-center gap-2.5 p-2.5 sm:gap-3 sm:p-3 rounded-xl bg-muted/40 border border-border/30 hover:border-border/60 hover:bg-muted/70 transition-all duration-200 cursor-pointer"
                    >
                        {/* Token Image */}
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                                ...
                                {token.is_graduated && (
                                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-500 font-medium flex-shrink-0">
                                        DEX
                                    </span>
                                )}
                                {!token.is_graduated && (
                                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-500 font-medium flex-shrink-0">
                                        CURVE
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="flex-shrink-0 text-right">
                            ...
                            <div className="text-[11px] text-muted-foreground">
                                {token.market_cap_usd && token.market_cap_usd !== "N/A" ? (
                                    <span className="text-emerald-500">MCap: {token.market_cap_usd}</span>
                                ) : null}
                            </div>
            ...
            {/* Pagination */}
            {/* Pagination - Simplified for Mobile, Range for Desktop */}
            {totalPages > 1 && (
...
export default NadFunTokenSearch;
