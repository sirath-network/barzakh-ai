...
const CHAIN_REGISTRY: ChainInfo[] = [
    ...
    // Aptos (non-EVM, 64-char hex)
    {
        ...
        keywords: ['aptos', 'apt', 'aptos wallet', 'petra wallet', 'pontem',
            'liquidswap', 'aries markets', 'thala labs', 'shelby', 'shelby storage', 'blob storage'],
        tokens: ['APT'],
        addressFormat: 'aptos',
        isEvm: false,
    },
    ...
    // Somnia Network (DreamDEX Event Contracts / Prediction Markets)
    {
        ...
        patterns: [
            ...
            /\bput\s+\d+(\.\d+)?\s*(tusdc)?\s*(on\s+)?(up|down)/i,
            /\b(up|down)\s+for\s+[A-Z0-9-]+/i,
            /\bpredict\s+(up|down)\b/i,
            /\b(live|active)\s*(prediction\s*)?market[s]?\b/i,
            /\b(view|show|check|open|my)\s*(dreamdex\s*)?(prediction\s*)?portfolio\b/i,
            /\b(active|open)\s*prediction[s]?\b/i,
            /\b(close|exit)\s*(my\s*)?(position|bet|prediction|market)?\b/i,
            /\bexit\s*(early|now|market)\b/i,
            /\bsell\s*(back\s*)?(contracts?|up|down)\b/i,
        ],
        keywords: ['somnia', 'dreamdex', 'dream dex', 'event contract', 'event contracts',
            'prediction market', 'prediction markets', 'live prediction markets', 'binary prediction', 'binary market',
            ...
            'close position', 'exit early', 'exit position', 'sell up', 'sell down',
            'view portfolio', 'show my dreamdex portfolio', 'dreamdex portfolio', 'live markets'],
        tokens: ['STT'],
        addressFormat: 'evm',
        isEvm: true,
    },
...
const INTENT_PATTERNS: IntentPattern[] = [
    ...
    // Aptos-specific
    {
        ...
        patterns: [
            ...
            /\b(shelby|storage|archive|save|store)\b.*\b(shelby|on-chain|aptos)\b/i,
            /\b(store|upload|archive|save)\b.*\b(this|message|file|content)\b.*\b(shelby|aptos)\b/i,
        ],
        keywords: [
            ...
            "thala labs", "aries markets", "econia", "liquidswap", "shelby", "shelby storage",
            "archive", "store on shelby",
        ],
        priority: 95,
    },
    ...
    // Somnia Network / DreamDEX Event Contracts (prediction markets)
    {
        ...
        keywords: [
            ...
            "live prediction markets", "live prediction market", "live markets",
