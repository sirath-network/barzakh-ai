...
function getClient(chainId: number): CdcAiAgentClient {
    if (!clientCache[chainId]) {
        // Try to get OpenAI key, or fallback to CometAPI key
        const apiKey = process.env.OPENAI_API_KEY || process.env.COMETAPI_API_KEY;

        if (!apiKey) {
            throw new Error("Missing API Key: OPENAI_API_KEY or COMETAPI_API_KEY is required for Crypto.com AI Agent SDK");
        }
