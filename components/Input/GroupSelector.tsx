...
const SelectionContent = ({ ...props }) => {
  ...
  const widthGreater = width > tailwindMd
  const expandToggle = isExpanded && widthGreater;
  ...
  return (
    <>
      <motion.div
        ...
        initial={false}
        animate={{
          width: expandToggle ? "auto" : "30px",
          gap: expandToggle ? "0.5rem" : 0,
          paddingRight: expandToggle ? "0.5rem" : 0,
        }}
        transition={{
          layout: { duration: 0.4 },
          duration: 0.4,
      ...
      >
        <AnimatePresence>
          {searchGroups.map((group, index) => {
            const showItem = expandToggle || props.selectedGroup === group.id;
            return (
              <motion.div
                key={group.id}
                animate={{
